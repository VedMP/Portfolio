  ...
  const handleCommand = async (cmdStr: string) => {
    ...
    switch (trimmed) {
      ...
      case "telemetry": {
        ...
        const online = typeof navigator !== "undefined" ? (navigator.onLine ? "ONLINE" : "OFFLINE") : "ONLINE";
        const navConn = typeof navigator !== "undefined" && "connection" in navigator ? (navigator as unknown as { connection?: { effectiveType?: string; downlink?: number } }).connection : undefined;
        const connType = navConn?.effectiveType ? navConn.effectiveType.toUpperCase() : "BROADBAND";
        const host = typeof window !== "undefined" ? window.location.host : "vedmpatel.me";

        output = (
          <div className="text-xs space-y-1 font-mono text-emerald-400">
            <div className="text-sky-400 font-bold">CLIENT SESSION TELEMETRY (SANDBOXED):</div>
            <div>Host Target: <span className="text-white">{host}</span></div>
            <div>Network Status: <span className="text-white">{online} ({connType})</span></div>
            <div>Logical Hardware Threads: <span className="text-white">{cores} cores</span></div>
            <div>Client Session Up-Time: <span className="text-white">{perfNow} ms</span></div>
            <div>Security Posture: <span className="text-emerald-400 font-semibold">Strict CSP / Zero Public Attack Surface</span></div>
            <div>Edge API: <span className="text-slate-400">Public telemetry endpoint disabled for defense-in-depth</span></div>
          </div>
        );
        break;
      }
